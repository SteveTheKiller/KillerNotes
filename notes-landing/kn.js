/* KillerNotes site - shared chrome behavior (theme, accent, language, easter egg).
...
(function () {
  ...
  var curAccent = 'purple';

  function applyAccent(name) {
    var theme = root.getAttribute('data-theme');
    var fam = famFor(theme);
    if (!ACCENTS[fam][name]) name = 'purple';
    curAccent = name;
    ...
    if (neutral) {
      ...
      try { localStorage.setItem('knotes-av', pair[0] + '|' + pair[1]); } catch (e) {}
    } else {
  ...
  function setTheme(name) {
    ...
    try { localStorage.setItem('knotes-theme', name); } catch (e) {}
  ...
  var LANGS = ['en','cs','es','de','fr','it','ja','tr','zh','zh-cn','bn','hu','pl','ru','kk'];
  var FLAGS = {
    ...
    kk: '<svg viewBox="0 0 24 24"><rect width="24" height="24" fill="#00afca"/><circle cx="13" cy="10.5" r="4" fill="#fec50c"/><polygon points="13,15.6 18,17.8 13,17.1 8,17.8" fill="#fec50c"/><rect width="2.2" height="24" fill="#fec50c"/></svg>'
  };
  ...
  // ---- Init ----
  var savedTheme = 'hc', savedAccent = 'purple', savedLang = 'en';
  try {
    savedTheme  = localStorage.getItem('knotes-theme')  || savedTheme;
    savedAccent = localStorage.getItem('knotes-accent') || savedAccent;
    savedLang   = localStorage.getItem('knotes-lang')   || savedLang;
  } catch (e) {}
