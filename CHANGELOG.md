...
### Added
- Hungarian and Polish localization for the complete app interface and killernotes.net, the eleventh and twelfth languages. Every one of the twelve now carries all 655 interface keys, so search and replace, the network-lock warnings, markdown notes, the markdown export, wikilinks and the link graph read in your own language rather than falling back to English.
